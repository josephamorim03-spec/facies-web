...
const ADMIN_PANEL = "rounded-control border border-edge bg-surface";
...
const ADMIN_CHIP = "rounded-control border border-edge bg-paper px-2 py-0.5 text-micro text-muted";
  ...
  return (
    <section className="space-y-4">
          ...
          {aiDrafts.map((draft) => {
            ...
            return <article key={`${draft.rollout_id}:${key}`} className={`${ADMIN_PANEL} p-4`}>
              ...
              <details className="mt-3">
                ...
                <textarea value={draftNotes[key] ?? ""} onChange={(event) => setDraftNotes((current) => ({ ...current, [key]: event.target.value }))} rows={2} placeholder="Nota editorial" className="mt-3 w-full rounded-control border border-edge bg-transparent p-2 text-sm" />
      ...
      {active ? <div className="fixed inset-0 z-40 bg-paper" onMouseDown={(event) => { if (event.target === event.currentTarget) setActive(null); }}><aside className="ml-auto h-full w-full overflow-y-auto bg-surface p-5 shadow-overlay sm:w-[min(760px,92vw)]"><div className="flex items-start justify-between gap-3"><div><p className="paper-eyebrow">Revisão versionada · v{active.question_version}</p><h2 className="mt-1 font-serif text-xl font-semibold">Decisão editorial</h2></div><button type="button" onClick={() => setActive(null)} className={ADMIN_BUTTON_SM}>Fechar</button></div>
        ...
        <div className="mt-5 space-y-2"><h3 className="font-semibold">Rubrica editorial</h3>{Object.entries(active.dimensions ?? {}).map(([key, dimension]) => <div key={key} className={`border p-3 ${stateTone(dimension.state)}`}><div className="flex items-center justify-between gap-2"><strong className="text-sm">{DIMENSION_LABELS[key] ?? key}</strong><span className="paper-eyebrow">{dimension.state}</span></div><p className="mt-1 text-sm">{dimension.summary}</p>{dimension.evidence.length ? <ul className="mt-2 list-disc pl-5 text-xs">{dimension.evidence.map((evidence) => <li key={evidence}>{evidence}</li>)}</ul> : null}</div>)}</div>
        <div className="mt-5"><div className="flex items-center justify-between"><h3 className="font-semibold">Diff proposto pela IA</h3><span className="text-xs text-muted">edite antes de aprovar</span></div><textarea value={patchText} onChange={(event) => setPatchText(event.target.value)} rows={10} spellCheck={false} className="mt-2 w-full rounded-control border border-edge bg-paper p-3 text-xs" /><p className="mt-2 text-xs text-muted">Evidências e incertezas permanecem no histórico mesmo quando o rascunho é rejeitado.</p></div>
        <textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} placeholder="Justificativa da decisão" className="mt-5 w-full rounded-control border border-edge bg-transparent p-3 text-sm" />
