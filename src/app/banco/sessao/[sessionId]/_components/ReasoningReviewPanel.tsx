...
export function ReasoningReviewPanel({ token, session, item, onSessionChange }: Props) {
  ...
  if (!review) {
    return (
      <div className="mt-4 rounded-lg border border-primary/35 bg-[var(--amber-tint)] p-4">
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          {item.reasoning_review_eligible && (
            <button
              ...
              className="rounded-lg border border-primary bg-primary px-4 py-2.5 text-sm font-semibold text-primaryInk disabled:opacity-50"
            >
              Revisar raciocínio
            </button>
          )}
          <button
            ...
            disabled={busy}
            onClick={() => void reveal()}
            className="rounded-lg border border-edge bg-surface px-4 py-2.5 text-sm font-semibold text-ink disabled:opacity-50"
          >
  ...
  return (
    <div className="mt-4 rounded-lg border border-primary/40 bg-surface p-4" aria-live="polite">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {RESPONSE_OPTIONS.map(([value, label]) => (
              <button
                ...
                className="rounded-lg border border-edge px-3 py-2 text-sm font-semibold text-ink hover:border-primary disabled:opacity-50"
              >
                {label}
              </button>
            ))}
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {Object.entries(review.attribution_options).map(([value, label]) => (
              <button
                ...
                className="rounded-lg border border-edge px-3 py-2 text-left text-sm font-semibold text-ink hover:border-primary disabled:opacity-50"
              >
                {label}
              </button>
            ))}
