...
export default function ExamQuestion({
  ...
  const detailsOpen = detailsPinned || detailsHover;
        ...
        <div className="relative mx-auto max-w-6xl px-4">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 py-2">
            ...
            <button
              ...
              aria-label={detailsOpen ? "Ocultar detalhes da sessão" : "Mostrar detalhes da sessão"}
              className="flex min-w-0 items-center gap-2 text-left"
            >
              <span className="shrink-0 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted">
                {sessionKindLabel}
              </span>
              <span className="max-w-[10rem] truncate font-serif text-sm font-semibold leading-tight text-ink md:max-w-[18rem]">
                {examLabel}
              </span>
              <IconChevronDown
                className={cx(
            ...
            <div className="h-1 w-16 overflow-hidden rounded-full bg-surfaceMuted" aria-hidden="true">
              <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} />
            </div>
            <div className="ml-auto flex flex-wrap items-center gap-1.5">
              <span
                className={cx(
                  "rounded-lg border px-2.5 py-1.5 font-mono text-sm font-semibold tabular-nums transition-colors",
                  isCritical
              ...
              <button
                type="button"
                onClick={onOpenMap}
                className="inline-flex items-center gap-1.5 rounded-lg border border-edge px-2.5 py-1.5 text-xs font-semibold text-muted hover:border-primary hover:text-ink"
              >
              ...
              {!finalized && (
                <button
                  ...
                  onClick={onFinalize}
                  disabled={busy}
                  className="rounded-lg border border-danger px-2.5 py-1.5 text-xs font-semibold text-danger hover:bg-danger hover:text-white disabled:opacity-50"
                >
          ...
          {detailsOpen && (
            <div className="absolute inset-x-0 top-full z-20 border-b border-t border-edge bg-surface px-4 py-3 shadow-lg">
            ...
            return (
              ...
              >
                <button
                  ...
                  disabled={busy || finalized || isEliminated}
