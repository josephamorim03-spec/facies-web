...
function phaseState(item: QuestionBankSessionItem, revealed: boolean): { label: string; detail: string; tone: Tone } {
  if (!item.answered && !item.selected_option) {
    return {
  ...
  if (!revealed && item.selected_option) {
    return {
...
export default function StudyQuestion({
  ...
  const canReveal = !finalized && Boolean(item.selected_option) && !revealed;
  const canCaptureAnswerSignals = !finalized && !revealed && !item.answer_committed;
            ...
            <section className="rounded-lg border border-edge bg-surface p-4">
              ...
              {revealed ? (
                <details className="mt-4">
                  ...
                  <div className="mt-2">
                    <ClinicalCyclePanel
                      answered={item.answered || Boolean(item.selected_option)}
                      revealed={revealed}
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        disabled={busy || finalized || revealed || item.answer_committed || isEliminated}
                      ...
                      {canEliminate && (
                        <button
                          ...
                          className={cx(
                            "flex w-11 shrink-0 items-center justify-center border-l border-edge transition-colors disabled:cursor-not-allowed disabled:opacity-45",
