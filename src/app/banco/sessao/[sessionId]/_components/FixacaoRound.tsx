    ...
    <div className="flex min-h-screen flex-col bg-paper">
        ...
        <div className="mx-auto max-w-4xl space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-control border border-edge bg-paper px-3 py-2 text-xs tabular-nums text-muted">
                {index + 1}/{total}
              </span>
              <button
                type="button"
                onClick={onExit}
                className="border border-edge px-3 py-2 text-xs text-muted transition-colors hover:border-primary hover:text-ink"
              >
                Sair
              </button>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="h-1.5 flex-1 overflow-hidden bg-surfaceMuted">
              <div className="h-full bg-primary transition-all" style={{ width: `${progress}%` }} />
            </div>
            <span className="w-10 text-right text-xs text-muted">{progress}%</span>
          </div>
        </div>
      </div>
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-6 md:px-6">
          ...
          {OPTIONS.map((option) => {
            ...
            return (
              <div
                key={option}
                className={cx(
                  "flex w-full items-stretch overflow-hidden border text-sm transition-colors",
                  isCorrectOpt
              ...
              >
                ...
                >
                  ...
                  {isCorrectOpt && <span className="hidden border border-current px-2 py-0.5 text-micro sm:inline">Gabarito</span>}
                </button>
                <button
                  type="button"
        ...
        {revealed && (
          <section
            className={cx(
              "mt-4 border bg-surface p-4 text-sm",
