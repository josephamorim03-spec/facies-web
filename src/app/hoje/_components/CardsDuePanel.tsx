...
function IconCards({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="butt" strokeLinejoin="miter" className={className} aria-hidden="true">
      <rect x="4" y="7" width="12" height="12"/>
      <rect x="8" y="5" width="12" height="12"/>
    </svg>
  );
}
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <Link
          ...
          className="border border-accent px-3 py-2 text-xs font-semibold text-accent transition-colors hover:bg-accent hover:text-accentInk"
        >
          Revisar
        </Link>
      </div>
